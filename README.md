...
**已知口径限制**: 标的池基于当前上市股票清单同步,已退市股票不包含在回测历史中(幸存者偏差),回测收益与胜率可能偏乐观,解读结果时请注意折扣。另外,ST 股的涨跌停档位判定依赖本地名称历史表(`data/instruments/name_history.parquet`,随每日标的同步自动累积);该表起始日期之前的历史行情按当前名称回溯,历史上摘帽/戴帽股票在早期的涨停判定可能有偏差,数据会随累积逐步自愈。
