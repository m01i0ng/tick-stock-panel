...
class QuoteService:
    ...
    def boot_check(self) -> None:
        """启动时检查 preferences: enabled 则自动启动; 从未设置且处于交易时段则自动开启。
        ...
        if preferences.get_realtime_quotes_enabled():
            self.start()
            return
        # 缺键才自动打开。disable() 会把 false 写进 preferences, 重启后保持关闭。
        if not preferences.realtime_quotes_explicitly_set() and self._should_auto_enable_on_boot():
            logger.info("首次启动且处于交易时段, 自动开启实时行情")
            self.start()

    def _should_auto_enable_on_boot(self) -> bool:
        """尚未写过开关时, 启动是否自动开启实时行情。

        时段判据用 _is_continuous_trading (与 status 的 is_trading_hours 同口径):
        _is_trading_hours 是轮询窗口, 含未完成的收盘定版, 工作日夜间也为真, 不可用于此处。
        交易日探针明确休市时不开; 探针未知时仍按周几近似。
        """
